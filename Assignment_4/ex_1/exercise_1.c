...
int main(int argc, char *argv) {
  ...
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, dimensions, NULL, &n_workitem, &workgroup_size, 0, NULL, NULL);
  */

  // 2D base case
  // Local work size = NULL, OpenCL implementation determines how to split local work items into appropriate instances
  /* 
  int dimensions = 2;
  size_t *work_dim = (size_t*) malloc(sizeof(size_t)*dimensions);
  work_dim[0] = 16;
  work_dim[1] = 16;
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, dimensions, NULL, work_dim, NULL, 0, NULL, NULL);
  */

  // 2D with 4x4 work groups
  /*
  int dimensions = 2;
  size_t *g_work_dim = (size_t*) malloc(sizeof(size_t)*dimensions);
  size_t *l_work_dim = (size_t*) malloc(sizeof(size_t)*dimensions);
  g_work_dim[0] = 16;
  g_work_dim[1] = 16;
  l_work_dim[0] = 4;
  l_work_dim[1] = 4;
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, dimensions, NULL, g_work_dim, l_work_dim, 0, NULL, NULL);
  */
  ...
  g_work_dim[1] = 8;
  g_work_dim[2] = 8;
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, dimensions, NULL, g_work_dim, NULL, 0, NULL, NULL);
