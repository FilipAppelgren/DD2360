...
int main(int argc, char *argv) {
  ...
  cl_mem x_dev = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, bytes, x, &err); 
  CHK_ERROR(err);

  cl_mem y_dev = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, bytes, y, &err);
  CHK_ERROR(err);

  //cl_mem a_dev = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float), NULL, &err); 
  CHK_ERROR(err);

  err = clEnqueueWriteBuffer(cmd_queue, x_dev, CL_TRUE, 0, bytes, x, 0, NULL, NULL);
  ...
  CHK_ERROR(err);
  err = clSetKernelArg (kernel, 3, sizeof (float), &n);
  CHK_ERROR(err);


  int dimensions = 1;
  size_t local_work_size = 256;    // Number of work items per work group

  int mul = ARRAY_SIZE/local_work_size;
  if(ARRAY_SIZE % local_work_size != 0){
    mul++;
  }

  size_t global_work_size = local_work_size * mul; // Total number of work items
  

  printf("size %d \n", local_work_size);
  err = clEnqueueNDRangeKernel(cmd_queue, kernel, dimensions, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
  CHK_ERROR(err);
  
  err = clEnqueueReadBuffer(cmd_queue, y_dev, CL_TRUE, 0, bytes, y, 0, NULL, NULL);
  CHK_ERROR(err);
