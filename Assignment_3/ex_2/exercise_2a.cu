...
{   
    ...
    if(pinned) {
            ...
            particles = (Particle*) malloc(bytes);
            gpu_particles = (Particle*) malloc(bytes);
    ...
    for(int i = 0; i < n_iterations; i++){
        cudaMemcpy(d_particles, particles, bytes, cudaMemcpyHostToDevice);
        timestep_update<<<grid_size, n_threads>>>(d_particles, n_particles);
        cudaMemcpy(gpu_particles, d_particles, bytes, cudaMemcpyDeviceToHost);
    }
